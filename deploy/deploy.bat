...
rh --c=server=%server.database%;uid=%username%;pwd=%password%;database=%dbName%; --s=%server.database% --d=%dbName% --f=%sql.files.directory% --vf=%version.file% --vx=%version.xpath% --r=%repository.path% --env=%environment% 
